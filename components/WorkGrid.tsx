    ...
    <section id="work" className="bg-transparent">
      <div className="container-page">
                ...
                <div className="relative aspect-[4/5] overflow-hidden rounded-[24px] bg-surface shadow-soft ring-1 ring-black/5 transition-[box-shadow,transform] duration-500 group-hover:-translate-y-1 group-hover:shadow-lift md:rounded-[28px]">
                  {item.type === "video" ? (
                    ...
                    <Image
                      ...
                      alt=""
                      fill
                      className={`object-contain p-6 transition-transform duration-700 group-hover:scale-[1.04] md:p-9 ${item.focus}`}
                      sizes="(max-width: 768px) 100vw, 50vw"
                    />
                  )}
                </div>
                <div className="mt-4 px-1 text-center md:mt-5">
                  <h3 className="text-lg font-medium tracking-tight text-ink transition-transform duration-300 group-hover:translate-y-0.5 md:text-xl">
                    {item.title}
                  </h3>
                </div>
              </article>
        ...
        <Reveal className="my-12 flex justify-center md:my-16">
          <p className="text-center text-[clamp(1.35rem,2.8vw,1.9rem)] font-medium tracking-tight text-ink/55">
            …and so much more.
          </p>
        </Reveal>
      </div>
